...
export default function EmployeeProfile() {
  ...
  const { data: files, isLoading: filesLoading } = useGetEmployeeFilesQuery(
    profile?.id || '',
    { skip: !profile }
  );
              ...
              <Grid item xs={12} md={8}>
                <form onSubmit={handleSubmit(onSubmit)}>
                  ...
                  <TextField
                    fullWidth
                    label="Salary"
                    value={formatPKR(profile?.salary || 0)}
                    InputProps={{ readOnly: true }}
                    sx={{ mb: 2 }}
                  />
                  <TextField
                  ...
                  <DatePicker
                    label="Date of Birth"
                    value={
                      control._getWatch('date_of_birth')
                        ? new Date(control._getWatch('date_of_birth'))
                        : null
                    }
                    onChange={(date) =>
                      control._setValue(
